var threeSum = function (nums) {
  // 오름차순 정렬
  // 최악의 경우 O(n2)
  // 우선 오름차순 정렬 0 인덱스가 1이상이면 false
  // 오름차순 정렬된 숫자들을 2중 for문으로 돌린다
  // 먼저 더해진 2개의 값을 0에서 뺀값이 있으면 true 반환이므로
  // 만약 0에서 뺀 값보다 커지면 반복문을 종료
  const answer = [];
  nums.sort((a, b) => a - b);
  // nums = [...new Set(nums)];
  if (nums[0] >= 1) {
  ...
  for (let i = 0; i < nums.length; i++) {
    ...
    for (let k = i + 1; k < nums.length; k++) {
      ...
      for (let j = k + 1; j < nums.length; j++) {
        if (0 - (firstNum + secondNum) === nums[j]) {
          answer.push([firstNum, secondNum, nums[j]]);
